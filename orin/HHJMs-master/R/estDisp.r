...
estDisp <- function(RespLog = list(Jlik1, Jlik2), 
  ...
  ff <- function(xx){  # xx are input values of dispersion parameters
    ...
    #-------------------------------New---------------------------------
    # if det(H) <=0, it will skip this set and try another one in the next iteration.
    deth <- det(H)
    if (!is.finite(deth) || deth <= 0) {
      if (Silent == FALSE) cat(" det(H) Illegal：", deth, "\n")
      return(1e10)  # punishment
    }
    #---------------------------------------------------------------------------
